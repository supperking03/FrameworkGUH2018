...
export default class CartoonScreen extends Component {
    constructor(props) {
        ...
        globalStore.register('CartoonScreen', (s) => this.setState(s),
            () => {
                return this.state
            });
    ...
    refresh = () => {
        ...
        if (this.state.next === null) {
            start = 0;
             ...
             start=(this.state.next> cartoonTest.length)?cartoonTest.length:this.state.next;
        }

        let end=(this.state.next+ this.state.limit> cartoonTest.length)?cartoonTest.length:start+this.state.limit;
        this.setState({videos: cartoonTest.slice(start,end),next:end});
    };
    ...
    render() {
        return (
            <View style={{flex: 1, backgroundColor: SURFACE_COLOR}}>
                ...
                <ImageBackground
                    resizeMode={"stretch"}
                    source={require('../../../src/assets/images/cartoonHeader.png')} style={{width:'100%',aspectRatio:5}}>
